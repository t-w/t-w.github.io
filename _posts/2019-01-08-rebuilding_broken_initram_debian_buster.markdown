...
the fact that I used UUIDs everywhere... Not sure if it was my fault, or Debian during
left something in configuration of encrypted devices...
...
have just SystemRescueCD on my network boot, so the standard rescue method does not work.
And there should be a standard way to do it with standard tools - right?

Having some experience with rebuilding those in the past (eg. modifying some live systems
to boot over network using NBD), I got gzip, xz, cpio - and got access only to something
very little, containing something like microcode.

Googling I found [the Debian page about initramfs][1.], which explained that the initramfs
has 2 parts - the first with microcode (cpio), and the second with "standard" initrd
(gzipped cpio).
